...
serve(async (req) => {
  ...
  try {
    ...
    // If redirect_url is provided, use it; otherwise use mobile app scheme
    const baseSuccessUrl = redirect_url || `${MOBILE_APP_SCHEME}payment/success`;
    const baseCancelUrl = redirect_url ? redirect_url.replace('success', 'cancel') : `${MOBILE_APP_SCHEME}payment/cancel`;
    
    // Append reference as query parameter if not already included
    const successUrl = baseSuccessUrl.includes('?') 
      ? `${baseSuccessUrl}&reference=${subscription_id}` 
      : `${baseSuccessUrl}?reference=${subscription_id}`;
    
    const cancelUrl = baseCancelUrl.includes('?') 
      ? `${baseCancelUrl}&reference=${subscription_id}` 
      : `${baseCancelUrl}?reference=${subscription_id}`;
    
    // Ensure callback URLs are properly formatted for Paystack (must be HTTPS)
    const formattedSuccessUrl = successUrl.startsWith('http') ? successUrl : `https://api.paystack.co/transaction/callback?redirect=${encodeURIComponent(successUrl)}`;
    const formattedCancelUrl = cancelUrl.startsWith('http') ? cancelUrl : `https://api.paystack.co/transaction/callback?redirect=${encodeURIComponent(cancelUrl)}`;
    ...
    const payloadToSend = {
      ...
      currency: currency,
      reference: reference,
      callback_url: formattedSuccessUrl,
      metadata: {
        cancel_action: formattedCancelUrl,
        subscription_id: subscription_id,
        customer_name: customer.first_name ? `${customer.first_name} ${customer.last_name || ''}` : customer.name,
        user_id: customer.id || metadata.user_id,
        ...metadata,
      }
    };
